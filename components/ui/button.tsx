...
/**
 * Button convention. Pick the variant by the action's role, and put it in the
 * place the reader already expects, so the same action lives in the same spot
 * on every page.
 *
 * Variant by role:
 * - `default` (solid)   the single primary action of a page or dialog
 *                       (Upload video, Create user/tag, Save, Add this device).
 * - `outline`           secondary actions that sit next to the primary one
 *                       (Watch source, Open in Transcribe, Refresh, Cancel).
 * - `secondary`         quiet toggles and the current tab in a nav group.
 * - `ghost`             low-emphasis, dense controls: icon buttons in table
 *                       rows, sort headers, toolbar items.
 * - `destructive`       delete / remove only. Never sit it directly beside a
 *                       benign button: keep it in an overflow (`⋯`) menu or a
 *                       confirm dialog's footer, visually set apart.
 * - `link`              inline text navigation inside prose.
 *
 * Placement:
 * - List and detail pages: the primary action lives in the page header, right
 *   aligned (`justify-between` with the nav/search on the left). Detail pages
